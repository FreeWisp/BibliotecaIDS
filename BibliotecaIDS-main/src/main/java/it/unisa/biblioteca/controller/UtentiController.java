 ...
 */
    ...
    private void handleSalva() {
        ...
        try {
            ...
            // Controllo matricola esistente (attivo / disattivato)
            Utente utenteConMatricola = biblioteca.getUtentiService()
                    .listaOrdinata()
                    .stream()
            ...
            if (utenteConMatricola != null) {

                // Caso 1: utente ATTIVO
                if (utenteConMatricola.isAttivo() &&
                    (!modalitaModifica || !matricola.equals(matricolaOriginale))) {

                    mostraErrore("Matricola già presente nel sistema!");
                    return;
                }

                // Caso 2: utente DISATTIVATO
                if (!utenteConMatricola.isAttivo() &&
                    (!modalitaModifica || !matricola.equals(matricolaOriginale))) {

                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                    alert.setTitle("Utente disattivato");
                    alert.setHeaderText("Matricola già esistente");
                    alert.setContentText(
                        "Questa matricola appartiene ad un utente disattivato.\n" +
                        "Vuoi riattivarlo?"
                    );
                    ...
                    if (risposta.isPresent() && risposta.get() == ButtonType.OK) {
                        // Riattivazione
                        utenteConMatricola.setAttivo(true);
                        biblioteca.getUtentiService().modifica(utenteConMatricola);
                        salvaJSON();
                        caricaUtentiAttivi();
                        lblStatus.setText("✅ Utente riattivato con successo!");
                    }

                    // In entrambi i casi blocca l'inserimento
                    return;
                }
            }
    ...
    private void handleDisattiva() {
        ...
        if (utente.isAttivo()) {
            boolean haPrestitiAttivi = biblioteca.getPrestitiService()
                .prestitiAttiviOrdinati()
                .stream()
                .anyMatch(p -> p.getMatricolaUtente().equals(utente.getMatricola()));

            if (haPrestitiAttivi) {
                mostraErrore(
                    "Non è possibile disattivare l'utente.\n" +
                    "Vi è almeno un prestito attivo associato all'utente in questione."
                );
                return;
            }
        }
