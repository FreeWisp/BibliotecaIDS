...
public class ServizioPrestiti {
    ...
    /**
     * @brief Restituisce gli utenti con i loro prestiti attivi
     * 
     * Crea una lista di oggetti UtenteConPrestiti, ognuno contenente
     * tutte le informazioni necessarie per visualizzare i prestiti
     * dell'utente in formato compatto.
     * 
     * @return Lista di utenti con i relativi prestiti
     */
    public List<UtenteConPrestiti> utentiConPrestiti() {
        List<Prestito> prestitiAttivi = prestitoReposit.cercaPrestitiAttivi();
        LocalDate oggi = LocalDate.now();

        // Raggruppa per matricola
        Map<String, List<Prestito>> prestitiPerMatricola = prestitiAttivi.stream()
            .collect(Collectors.groupingBy(Prestito::getMatricolaUtente));

        List<UtenteConPrestiti> risultato = new ArrayList<>();

        for (Map.Entry<String, List<Prestito>> entry : prestitiPerMatricola.entrySet()) {
            String matricola = entry.getKey();
            List<Prestito> prestiti = entry.getValue();

            // Trova info utente
            List<Utente> utentiTrovati = utenteReposit.cercaPerMatricola(matricola);
            String nomeCompleto = utentiTrovati.isEmpty() 
                ? "Utente Sconosciuto" 
                : utentiTrovati.get(0).getCognome() + " " + utentiTrovati.get(0).getNome();

            UtenteConPrestiti ucp = new UtenteConPrestiti(matricola, nomeCompleto);

            // Aggiungi tutti i prestiti
            for (Prestito p : prestiti) {
                // Trova info libro
                List<Libro> libriTrovati = libroReposit.cercaTramiteIsbn(p.getIsbnLibro());
                String titolo = "ISBN: " + p.getIsbnLibro();
                String autore = "";

                if (!libriTrovati.isEmpty()) {
                    titolo = libriTrovati.get(0).getTitolo();
                    autore = libriTrovati.get(0).getAutore();
                }

                boolean inRitardo = p.getDataRestituzione().isBefore(oggi);

                ucp.aggiungiPrestito(titolo, autore, p.getDataRestituzione(), inRitardo);
            }
        ...
        // Ordina per cognome
        risultato.sort(Comparator.comparing(UtenteConPrestiti::getNomeCompleto));
